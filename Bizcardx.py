...
import numpy as np
import re
import psycpog2.connector
import io
# connect the database

mydb = psycopg2.connect(
    host='127.0.0.1',  # Replace with the actual IP address
    user='5432',
    password='1234')
...
from PIL import Image
def new_func():
    image = Image.open("C:\\Users\\Ganesh Babu\\Desktop\\BizCardX_ Extracting Business Card Data with OCR\\download (4).jpg")
    return image

icon = new_func()
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR | By SHAKTHI SIVANI G",
                   page_icon=icon,
                   layout="wide",
                   initial_sidebar_state="expanded",
                   menu_items={'About': """# This OCR app is created by *SHAKTHI SIVANI.G*!"""})
...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background:url("C:\\Users\\Ganesh Babu\\Desktop\\BizCardX_ Extracting Business Card Data with OCR\\picture");
...
if selected == "Home":
    ...
    with col1:
        st.image(Image.open("C:\\Users\\Ganesh Babu\\Desktop\\BizCardX_ Extracting Business Card Data with OCR\\download (4).jpg"), width=500)
